...
enum NodeType { N_LEAF, N_CAT, N_FASTCAT, N_PARALLEL, N_IDX, N_TIMES, N_DIVIDE, N_RANDOM, N_REPLICATE, N_ELONGATE, N_EUCLID, N_RANDOMALT, N_POLY, N_LAST };
static const char *node_type_names[N_LAST] = {"LEAF",   "CAT", "FASTCAT", "PARALLEL", "IDX",    "TIMES", "DIVIDE",
                                        "RANDOM", "REPLICATE",  "ELONGATE",    "EUCLID", "RANDOMALT",   "POLY"};
...
typedef struct Sound {
    ...
    Wave *waves; // stb_ds array, indexed by idx or note
} Sound;

struct { char *key; Sound *value; } *g_sounds;
...
typedef struct Node {
    int32_t type;
    int32_t start, end;  // half-open [start,end) character range
    int32_t first_child; // index or -1
    int32_t next_sib;    // index or -1
    float total_length; // total length of all children
    Value value; // for a leaf, the value; for others, maximum number value of all children.
} Node;
...
void pretty_print(const char *src, Node *nodes, int i, int depth) {
    ...
    printf("%d - %s - maxlen %g val %g %s '%s'\n", i, node_type_names[nodes[i].type], nodes[i].total_length, nodes[i].value.number, print_midinote(nodes[i].value.note), nodes[i].value.sound?nodes[i].value.sound->name:"");
...
static int parse_args(Parser *p, int group_type, int start, int is_poly) {
    ...
    while (!isclosing(peek(p))) {
        ...
        if (i == -1)
            return -1;
        if (group_link_idx <0) p->nodes[group_node].first_child = i;
        else p->nodes[group_link_idx].next_sib = i;
        group_link_idx = i;
        ...
        if (is_comma_or_pipe) {
            ...
            skipws(p);
            if (comma_node < 0) {
                comma_node = make_node(p, (is_poly) ? N_POLY: comma_char==','?N_PARALLEL:N_RANDOMALT, group_node, -1, start, start);
...
static int parse_op(Parser *p, int left_node, char op, int node_type, int optional_right) {
    ...
    consume(p, op);
    int right_node;
    if (optional_right && (isdelimiter(peek(p)))) {
        right_node=-1;
    } else {
    ...
    switch (peek(p)) {
        case '_': // shorthand for elongate. we only allow it to be a single character
            if (isdelimiter(peek_i(p, p->i+1))) {
                ...
                return elongate_node;
            }
            break;
        case '*':
            return parse_op(p, node, '*', N_TIMES, 0);
        case ':':
            return parse_op(p, node, ':', N_IDX, 0);
        case '/':
            return parse_op(p, node, '/', N_DIVIDE, 0);
        case '?':
            return parse_op(p, node, '?', N_RANDOM, 1);
        case '!':
            return parse_op(p, node, '!', N_REPLICATE, 0);
        case '@':
            return parse_op(p, node, '@', N_ELONGATE, 0);
        case '(':
            return parse_euclid(p, node);
        case '%':
...
/*
static int count_children(Parser *p, int node) {
    int count = 0;
    for (int i = p->nodes[node].first_child; i >= 0; i = p->nodes[i].next_sib) {
        p->nodes[i].num_children = count_children(p, i);
        count++;
    }
    return count;
}*/
...
static int get_nth_child(Parser *p, int nodeidx, int n) {
    for (int i = p->nodes[nodeidx].first_child; i >= 0; i = p->nodes[i].next_sib) {
        if (n==0) return i;
        n--;
    }
    return -1;
...
static Hap *make_haps(Parser *p, int nodeidx, float t0, float t1, float tscale, float tofs, Hap **haps) {
    if (nodeidx<0) return *haps;
    Node *n= p->nodes+nodeidx;
    switch (n->type) {
        case N_DIVIDE: 
        case N_TIMES: {
            if (n->first_child<0) return *haps;
            Node *nleft = p->nodes+n->first_child;
            float speed_scale = 1.f;
            if (nleft->next_sib>=0)
                speed_scale = p->nodes[nleft->next_sib].value.number;
            if (speed_scale<=0.f) speed_scale=1.f;
            if (n->type == N_DIVIDE)
                speed_scale = 1.f / speed_scale;
            //printf("speed - from %f to %f, scale %f\n", t0, t1, speed_scale);
            return make_haps(p, n->first_child, t0 * speed_scale, t1 * speed_scale, tscale / speed_scale, tofs, haps);
        break; }
        case N_FASTCAT: 
            t0 *= n->total_length;
            t1 *= n->total_length;
        ...
        case N_CAT: {            
            ...
            while (from < t1) {
                if (child>=0) {
                    ...
                    if (child_length<=0.f) child_length=1.f;
                    ...
                    if (from < t1 && to > t0) {
                        ...
                        make_haps(p, child, child_from, child_to, tscale, tofs+(from-child_from)*tscale, haps);
                    }
                    from=to;
                    ...
                    float to = from + 1.f;
                    if (from < t1 && to > t0) {
                        Hap h = (Hap){from * tscale + tofs, to * tscale + tofs, nodeidx};
                        // printf("%f %f - node %d %.*s:%d\n", h.t0, h.t1, h.node, n->end-n->start, p->s+n->start, h.idx);
                        stbds_arrput(*haps, h);     
                    }
                    from=to;
                    loop_index++;
        ...
        case N_PARALLEL: {
            int child = n->first_child;
            while (child>=0) {
                make_haps(p, child, t0, t1, tscale, tofs, haps);
                child = p->nodes[child].next_sib;
            }
            break;
        }
        // case N_LEAF: {
        //     Hap h = (Hap){t0 * tscale + tofs, t1 * tscale + tofs, nodeidx};
        //     //printf("%f %f - node %d %.*s:%d\n", h.t0, h.t1, h.node, n->end-n->start, p->s+n->start, h.idx);
        //     stbds_arrput(*haps, h);
        //     break; }
    }
...
void test_minipat(void) {
    //const char *s = "[sd,<oh hh>,<bd bd rim>]";
    //const char *s = "{c eb g, c2 g2}%4";
    const char *s = "[bd <hh oh>,rim*8]";
    printf("\nparsing " COLOR_BRIGHT_GREEN "\"%s\"\n" COLOR_RESET "\n", s);
    Parser p = parse(s, strlen(s));
    pretty_print(s, p.nodes, p.root, 0);
    Hap *haps = NULL;
    make_haps(&p, p.root, 0.0f, 4.0f, 1.0f, 0.f, &haps);
    printf("      time 0   |   |   |   v   |   |   |   1   |   |   |   v   |   |   |   2   |   |   |   v   |   |   |   3   |   |   |   v   |   |   |   4\n");
    for (int i = 0; i < stbds_arrlen(haps); i++) {
        Node *n = &p.nodes[haps[i].node];
        int j;
        for (j=0;j<i;++j) {
            Node *nj = &p.nodes[haps[j].node];
            if (n->value.sound == nj->value.sound) break;
        }
        if (j<i) continue;
        printf("%10s ", n->value.sound?n->value.sound->name:"");
        char line[129]={};
        for (int j=0;j<128;++j) line[j]='.';
        for (int j=i; j<stbds_arrlen(haps);++j) {
            Node *nj= &p.nodes[haps[j].node];
            if (nj->value.sound != n->value.sound) continue;
            int x1=(int)(haps[j].t0*32.f);
            int x2=(int)(haps[j].t1*32.f);
            for (int x=x1;x<x2;++x) {
                if (x>=0 && x<128) line[x]=(x==x1)?'*':(x==x2-1)?'>':'=';
            }
        }
        printf("%s\n", line);
    }
