...
vec4 pixel(vec2 uv) {
    ...
    for (int smpl = 0; smpl<16;smpl++) {
        ...
        for (int bounce = 0; bounce < 3; ++bounce) {
            float tmax = 1e9;
            vec3 n=vec3(0.);
            vec2 spht = sphere_intersect(ro, rd, 1.);
            ...
            plane_intersect(ro,rd,vec4(0,0,1,-1),tmax,n);
            plane_intersect(ro,rd,vec4(0,1,0,-1),tmax,n);
            plane_intersect(ro,rd,vec4(1,0,0,-1),tmax,n);
            if (tmax==1e9) {
                //c+=skycol(rd,0.) * thpt;
                break;
            } else {
                ro = ro + rd * tmax;
                if (ro.y>2.5 && ro.y<3.3) c+=1.;
                r4=rnd4();
